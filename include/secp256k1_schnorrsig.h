...
/** Opaque data structure that holds a parsed ComSig signature.
 *
 *  The exact representation of data inside is implementation defined and not
 *  guaranteed to be portable between different platforms or versions. It is
 *  however guaranteed to be 96 bytes in size, and can be safely copied/moved.
 *  If you need to convert to a format suitable for storage, transmission, or
 *  comparison, use the `secp256k1_comsig_serialize` and
 *  `secp256k1_comsig_parse` functions.
 */
typedef struct {
    unsigned char data[96];
} secp256k1_comsig;

/** Serialize a ComSig signature.
 *
 *  Returns: 1
 *  Args:    ctx: a secp256k1 context object
 *  Out:   out96: pointer to a 96-byte array to store the serialized signature
 *  In:      sig: pointer to the signature
 *
 *  See secp256k1_comsig_parse for details about the encoding.
 */
SECP256K1_API int secp256k1_comsig_serialize(
        const secp256k1_context* ctx,
        unsigned char* out96,
        const secp256k1_comsig* sig
) SECP256K1_ARG_NONNULL(1) SECP256K1_ARG_NONNULL(2) SECP256K1_ARG_NONNULL(3);

/** Parse a ComSig signature.
 *
 *  Returns: 1 when the signature could be parsed, 0 otherwise.
 *  Args:    ctx: a secp256k1 context object
 *  Out:     sig: pointer to a signature object
 *  In:     in96: pointer to the 96-byte signature to be parsed
 *
 * The signature is serialized in the form R||u||v, where R is a 32-byte public
 * key (x-coordinate only; the y-coordinate is considered to be the unique
 * y-coordinate satisfying the curve equation that is a quadratic residue)
 * and both u and v are a 32-byte big-endian scalar.
 *
 * After the call, sig will always be initialized. If parsing failed or the
 * encoded numbers are out of range, signature validation with it is
 * guaranteed to fail for every message and public key.
 */
SECP256K1_API int secp256k1_comsig_parse(
        const secp256k1_context* ctx,
 ...
 *  Args:    ctx: pointer to a context object, initialized for signing (cannot be NULL)
 *  Out:     sig: pointer to the returned signature (cannot be NULL)
 *        pubkey: pointer to the returned public key (can be NULL)
 *       nonce_is_negated: a pointer to an integer indicates if signing algorithm negated the
 *                nonce (can be NULL)
 *  In:    msg32: the 32-byte message hash being signed (cannot be NULL)
 *        seckey: pointer to a 32-byte secret key (cannot be NULL)
 *         value: a 32-byte value to commit to.
 *       noncefp: pointer to a nonce generation function. If NULL, secp256k1_nonce_function_bipschnorr is used
 *        algo16: pointer to a seed which is used for noncefp (cannot be NULL if noncefp is NULL)
 */
SECP256K1_API int secp256k1_comsig_sign(
        const secp256k1_context* ctx,
        secp256k1_comsig* sig,
 ...
 *        pubkey: pointer to a public key to verify with (cannot be NULL)
 */
SECP256K1_API SECP256K1_WARN_UNUSED_RESULT int secp256k1_comsig_verify(
        const secp256k1_context* ctx,
        const secp256k1_comsig* sig,
        const unsigned char* msg32,
        const secp256k1_pubkey* pubkey
